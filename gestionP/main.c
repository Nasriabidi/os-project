...
{
    char code[30];  // Unique identifier for the process 
    int date_arr; // Arrival date of the process 
    int dur_exec_non_modif_proc;  // Duration of execution for the process without modifications
    int dur_exec_modif_proc;  // Duration of execution for the process with modifications
    int dur_non_modif_es; // Duration of non-modification-related E/S for the process
    int dur_modif_es; // Duration of modification-related E/S for the process
    int priorite;
    int dur_pret; // Duration the process is in the ready state
    int dur_blq; // Duration the process is in the blocked state
    int turn;   // Turnaround time 
    int etat;  // State of the process
    comment cmt[maxim]; // Array of comments
    struct processus *suiv;   // Pointer to the next process in the list
}
processus ;  // Creating an processus for the struct to simplify the declaration of variables of this type
...
// verify the structure of a file (he number semicolons, spaces.. ) 

int verif_bcp(char nom_fich[])
{
    FILE *p=fopen(nom_fich,"rt");  // Open the file  
    int c,cmpt_poin_vrg=0,cmpt_lign=0;  // Variables to count characters, semicolons, and lines
    char ch[100];
    ...
    else
    {
        ...
        while(fgets(ch,sizeof(ch),p)!=NULL)     // Count the non-empty lines in the file
        {
            if(strcmp(ch,"\n")!=0)
                cmpt_lign++;
        }
        fclose(p);  // Display the number of lines 
         printf("%d",cmpt_lign);
         getch();

        p=fopen(nom_fich,"rt");

        while((c=fgetc(p))!=EOF) // Check characters in the file for semicolons and EOF stands for End of File
        {
            if(c==';')
            {
                cmpt_poin_vrg++;
            }
            else  // Check for spaces and display a message if found
            {
                if(c==' ')
                {
     ...
     // Check if the file is empty 
        ...
        {
            if(cmpt_poin_vrg!=(4*cmpt_lign))    // Check if the number and position of semicolons is correct  
            {
...
// Fonction pour formater et afficher une ligne de tableau
void fin_tab(char *a,int b,int c,int d,int e)
{
    ...
    putchar(179);
    printf(" ");
   // Affichage de la partie contenant la chaîne de caractères a
    for(i=1; i<=(9-aa)/2; i++)
        printf(" ");
    printf("%s",a);
    for(i=1; i<=((9-aa)/2)+(9-aa)%2; i++)
...
printf("\n");
printf("\t\t\t    +-------------------------------------------------------------+\n");
printf("\t\t\t    |            MENU DES ALGORITHMES D'ORDONNANCEMENT            |\n");
printf("\t\t\t    +-------------------------------------------------------------+\n");
printf("\t\t\t    | 1. First-Come-First-Served (FIFO)                           |\n");
printf("\t\t\t    | 2. Round Robin with Time Quantum=1                          |\n");
printf("\t\t\t    | 3. Priority with Preemption                                 |\n");
printf("\t\t\t    | 4. Shortest Job First (SJF) with Blocked State              |\n");
printf("\t\t\t    | 5. Round Robin with Time Quantum=1 and SJF for Blocked State|\n");
printf("\t\t\t    | 6. Priority with Preemption and SJF for Blocked State       |\n");
...
int main()
{
   ...
   printf("\t\t\t       Bienvenue dans le Programme de Gestion des Processus\n");
   printf("\t\t\t Ce programme gère les processus en fonction du type d'ordonnancement .\n\n");
